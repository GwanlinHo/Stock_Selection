...
啟動全流程掃描。適合每週五或週日執行。
*   **動作**: `uv run main.py --mode full` + AI 分析 + **Git 同步 (GitHub)**。

### 2. `stock selection skip` (跳過海選)
跳過 2000 檔標的的技術面掃描，直接讀取上一次的 L2 結果進行籌碼與基本面精煉。
*   **動作**: `uv run main.py --mode skip-scan` + AI 分析 + **Git 同步 (GitHub)**。

### 3. `stock selection report` (僅重新產生報告)
完全不抓取新數據，直接讀取上一次的精選結果產出 Markdown 週報。
*   **動作**: `uv run main.py --mode report-only` + AI 分析 + **Git 同步 (GitHub)**。
...
*   **強制同步**: 每次產生新的報告（.md）與資料暫存（.json）後，**必須執行 git add/commit/push 將成果同步至 GitHub 儲存庫**。
